...
- **Due date** — when you set one, an **all-day** event automatically appears on the Calendar (and on Google Calendar). Click the **Clear** chip next to the date to remove it (also clears the calendar entry).
- **Drive folder** — link a Google Drive folder by clicking **Pick from Drive**. The picker **opens inside your shared supervision Drive folder** (the one your supervisor set up for you), not in the whole "My Drive". You can browse subfolders, pick one, or use the *Browse my Drive instead* escape if you really need to attach something outside it. Once linked, an **Open Drive folder** button appears in the task and the task card shows a small folder icon. The same field is also available on **calendar events** — when the event is tied to you, the picker opens inside your folder too.
- **Assignee** — usually you
- **Subtasks** — a small checklist inside the task; tick items as you go. The card shows a `2/5` style progress badge. Each subtask can also be given its **own deadline** (date picker next to it). A subtask's deadline can't be later than the task's own deadline — if you pick a later date you'll get an error and it won't save. Any subtask that has a deadline also shows up as an **all-day** `[Sub-task]_… · <task>` entry on the Calendar **and is pushed to Google Calendar** (subtasks with no deadline don't appear on the calendar). On hover, each subtask row exposes two distinct controls: **Clear** (drops just the deadline, keeps the subtask) and the **🗑 Trash** icon (deletes the subtask).
...
- **Links** — a free-form list of external URLs attached to the task: an Overleaf paper, a useful blog post, a GitHub repo, a Zotero entry. Each link has a **Label** (what shows in the UI) and a **URL** (auto-prefixed with `https://` if you forget). Add as many as you need; hover a link to **Edit** or **Remove** it. The same Links section is also available on **calendar events** (open an event → scroll past the description). It's separate from the **Drive folder** field, which still has its dedicated Pick-from-Drive button.
...
- **Task events** — every task with a due date appears as an **all-day** outlined chip with a small circle icon and a single-letter priority tag (L/M/H/U). Its name is prefixed **`[Task]_`** (and sub-task deadlines show as **`[Sub-task]_…`** with a dashed border); both kinds also sync to Google Calendar so deadlines land in the same place you keep your meetings. Click it to see the task (sub-tasks open their parent task) in a quick view **without leaving the Calendar**; close it and you're still on the Calendar. Use **Open in Tasks board** in that view to go to the full task on the Tasks board.
...
- **You can start your own topic** (it's always **whole-team** visible) and join any discussion shared with you. Topics a supervisor marks *Supervisors only* stay hidden from students — you won't see or create those.
...
- **Alternate emails** — for information only (like LinkedIn / ORCID). Add any number of secondary email addresses (personal Gmail, institutional address, etc.); they appear on your profile alongside your primary email. Notifications and login still use your primary Google account. *Two places to set them:* (1) **Settings → My profile** (lives on your user account, used everywhere your name shows up), and (2) the **student profile** Edit dialog under the *Alternate emails* field (lives on your student record, shown specifically on your student-profile header next to your primary email). Either or both — they're independent informational fields, mirroring how LinkedIn / ORCID exist in both places.
...
Stuck? Open **Help** (❓ in the sidebar) any time. It shows this whole student manual **right inside the app**, with a **contents list** down the side so you can jump straight to any section, plus a **Slide overview** you can download for a quick visual tour. Nothing to install and nothing to keep up to date — it's always the current version.
