...
- **Project Researcher** — a postdoc / researcher embedded on a project *with* the student. **Read-only** on the student (views tasks, events, files and calendar and can comment, but can't edit them) and — unlike a team advisor — **never sees private supervisor notes or wellbeing**. They get the student's shared Drive folder and supervision calendar **view-only**. They also set up **their own** Drive folder + calendar themselves (from **Settings → My workspace**, in their own Google account), view-only for others: the **folder** is shared with the students they work with and each of those students' supervisors + team advisors (not external advisors / committee), so the student and researcher can see each other's files without editing — it shows for you in the **Files** module under **Drives → Researchers**; the **calendar** is shared with the **whole senior team** so everyone can see their schedule — and its events show up **read-only inside the Calendar module** (in the researcher's colour) for the senior team, alongside the app's own events. Not part of the senior team (no My Work).
...
**Notifications.** Two surfaces: the **My Work** sidebar entry shows a **teal dot / count** when a teammate has, since you last opened it, shared a new item, updated a shared item, or commented on something you can see (opening My Work clears it); and the **🔔 bell** now folds in My Work activity too — newly **shared items** and **comments** on items you can see appear in the bell feed and click through to My Work (senior team only). Comments on your own shared item additionally notify you by email if email is configured.
...
Open **Help** (❓ in the sidebar) whenever you need it. It renders the full manual for your role **inside the app** — as a supervisor you get this guide **and** the student guide (so you can see exactly what your students see), each with a **contents list** for jumping around and a **Slide overview** (.pptx) you can download. It always reflects the current version of the app, so there's no separate PDF to chase.
