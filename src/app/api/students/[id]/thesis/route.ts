import { NextResponse } from "next/server";
import { z } from "zod";
import { auth } from "@/auth";
import { prisma } from "@/lib/prisma";
import { teamLevelForStudent, type Role } from "@/lib/access";

const Body = z.object({
  title: z.string().min(1),
  status: z.string().optional(),
  driveUrl: z.string().nullable().optional(),
  notes: z.string().nullable().optional(),
});

export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user) return NextResponse.json({ error: "unauth" }, { status: 401 });
  const { id } = await params;
  const level = await teamLevelForStudent(id, session.user.id, session.user.role as Role);
  if (level === null) return NextResponse.json({ error: "not found" }, { status: 404 });

  const chapters = await prisma.thesisChapter.findMany({
    where: { studentId: id },
    orderBy: [{ order: "asc" }, { createdAt: "asc" }],
  });
  return NextResponse.json({ chapters, canWrite: level === "supervisor" || level === "self" });
}

export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user) return NextResponse.json({ error: "unauth" }, { status: 401 });
  const { id } = await params;
  const level = await teamLevelForStudent(id, session.user.id, session.user.role as Role);
  if (level !== "supervisor" && level !== "self")
    return NextResponse.json(
      { error: "Only supervisors can edit the thesis plan" },
      { status: 403 },
    );

  const parsed = Body.safeParse(await req.json().catch(() => null));
  if (!parsed.success)
    return NextResponse.json(
      { error: parsed.error.issues[0]?.message ?? "bad input" },
      { status: 400 },
    );
  const d = parsed.data;

  const last = await prisma.thesisChapter.findFirst({
    where: { studentId: id },
    orderBy: { order: "desc" },
    select: { order: true },
  });
  const chapter = await prisma.thesisChapter.create({
    data: {
      studentId: id,
      title: d.title,
      status: d.status ?? "planned",
      driveUrl: d.driveUrl ?? null,
      notes: d.notes ?? null,
      order: (last?.order ?? -1) + 1,
    },
  });
  return NextResponse.json({ chapter });
}
