...
- **Per-message "Seen by …" indicator in group chat channels.** Today only the unread count rolls back when a user opens a channel; the author can't see *who* on a 3+ member channel has read their latest message. **Data is already there** — `ChannelMember.lastRead` is updated on channel open, so any message with `createdAt <= member.lastRead` has been "seen" by that member. **Design (when picked up):** show a compact "Seen by Pablo · Javier" / "Seen by N" chip below the *last* message of each read-state burst (don't repeat on every line). Decide at build time: (a) show on 1:1 channels too or only ≥3 members; (b) include / exclude the message author from the readers list; (c) tooltip with per-reader timestamp on hover. Approximation caveat to call out: `lastRead` is updated on channel open (not per-message scroll), so a user who opened the channel briefly and left is counted as having read everything visible — same approximation as WhatsApp/Slack, fine for now. **Effort:** ~45 min (one API tweak to expose `members[].lastReadAt`, one render block in `chat-view.tsx`). **No schema change.**

- **Agent-driven Feedback triage (three escalating options).** Idea: a Claude agent reads open Feedback tickets, classifies them (bug / question / feature request / out-of-scope), proposes a diagnosis, and optionally attempts the fix on a worktree → PR. The Feedback schema already has everything we need (`Feedback.{kind,subject,body,status,imageUrl}`, threaded `FeedbackMessage[]`, status workflow `open → planned → in_progress → done | declined`) — no migration. Three architectures, pick whichever fits when picked up:

  - **A. Manual on-demand — `/triage-feedback` slash command (start here).** A new admin-only endpoint `/api/admin/feedback/open` returns open tickets + thread messages + screenshot URL. A skill walks the human through each ticket and offers per-ticket actions (*diagnosis only* / *draft a fix in a worktree, run tsc, show diff* / *skip*). Human decides whether to ship. **Effort:** ~2 hours. **Cost:** only when invoked. **Risk:** zero — fully under control. Best starting point because it lets us calibrate the triage prompt on real tickets before automating anything.

  - **B. Nightly Routine — read-only triage.** Same logic as A but on a cron (Claude Code Routines via `mcp__scheduled-tasks`, e.g. daily 02:00). Each night: fetch tickets that became `open` since the last run (dedupe via a marker file in the repo), write a short diagnosis for each, output a `FEEDBACK_TRIAGE.md` on a branch and open a PR. Optionally: post the diagnosis as a *draft* admin reply on the ticket (saved to the thread but not sent — the human clicks Reply to send). **Effort:** ~4 hours. **Cost:** ~$0.10–$0.50/night for a handful of tickets. **Risk:** low — read-only; only writes a triage doc + draft replies that need human approval.

  - **C. Nightly Routine — auto-PR for "simple" classifications.** B plus: when the agent classifies a ticket as a "simple fix" with high confidence, spawn a worktree, attempt the change, `tsc --noEmit`, open a code PR (one per ticket), and link back to the feedback ticket from the PR description. **Effort:** ~1 day (extra: worktree management, PR opening, hard guardrails on what counts as "simple"). **Risk:** moderate — auto-PR is fine since it can't merge itself, but a flood of low-quality PRs is annoying. Need confidence thresholds, a per-ticket attempt cap, and a hard token cap per attempt (~30K) to bail on flailing.

  - **Hard guardrails for all three:** never push to main (PRs only); never auto-reply to feedback authors (drafts only); never auto-close a ticket; one attempt per ticket per agent run; hard token cap per attempt; don't send screenshots or ticket bodies to any service outside the agent's own trusted scope.
