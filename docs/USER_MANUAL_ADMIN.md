...
- **Undo / soft-delete (Tasks)** — `Ticket.archivedAt` (additive migration). Task DELETE sets `archivedAt` instead of removing; `/api/tickets/[id]/restore` clears it + re-syncs the due event. **All task read queries were audited and filtered** for `archivedAt: null` (tickets list/kanban/dashboard/team/student-profile, incl. relation `_count`). Undo toast on the board (7s). **Scoped to Tasks only** (highest-regret deletion); other models + an automated purge of old archived rows are a tracked follow-up (see IMPROVEMENT_PLAN.md §14) — deliberately not a one-pass app-wide sweep to limit regression risk. To hard-purge: `DELETE FROM "Ticket" WHERE "archivedAt" < now() - interval '30 days'` in Neon.
- **Real notifications** — `Notification` table + `src/lib/notify.ts` (`notify(userIds, {type,message,link,actorId})` inserts in-app rows, skips the actor, best-effort emails via Resend if `RESEND_API_KEY` set). 🔔 bell in the topbar (`notification-bell.tsx`) polls `/api/notifications` every 30s for unread count + list, mark-one / mark-all read. Triggers wired: task assigned (ticket create w/ assignee), new comment on a task (notifies assignee + creator). In-app always works; email only with `RESEND_API_KEY`. Web/browser push deferred. This is also the general "bubble number" mechanism for cross-user updates.
...
- **Reading list** — a top-level **module** (`/reading`, own sidebar entry, visible to all roles): `ReadingItem` table. Students propose (status `proposed`) → supervisors approve/reject; supervisors add auto-approved. Status flow proposed→approved→reading→done (or rejected). Per-student API `/api/students/[id]/reading`; the module page aggregates across the viewer's visible students and computes per-student `teamLevelForStudent` to decide who can approve vs propose. Advisors/committee read-only.
