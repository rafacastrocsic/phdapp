...
- **Email-style comment attachments (Discussions only)** (user request) — a discussion comment can carry **text + images + documents**. Additive `Comment.attachments` (JSON `[{name,url,mimeType,size}]`; migration `20260720130000_comment_attachments`). New `POST /api/discussions/upload` → Vercel Blob under the **`discussions/` prefix with NO auto-cleanup** (permanent — contrast chat's 7-day purge), 25 MB, same safe image/office/pdf mime set as chat. The comment `POST` validates attachments via `src/lib/comment-attachments.ts` (host-checks each URL ends with `blob.vercel-storage.com`, so a crafted request can't embed arbitrary external URLs; attachment-only comments allowed); `GET/PATCH` return them. `CommentsThread` gained **opt-in** `enableAttachments` + `uploadUrl` props (default off ⇒ task/event/reading composers byte-for-byte unchanged); when on, a multiline composer with an Attach button + drag-and-drop, removable pending previews, inline image thumbnails + document download chips. Enabled only on the Discussions detail page.
